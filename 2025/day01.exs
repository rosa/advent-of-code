...
defmodule Rotations do
  ...
  def rotate({"L", n}, current), do: Integer.mod(current - n, 100)
  def rotate({"R", n}, current), do: Integer.mod(current + n, 100)

  def pass({"L", n}, current), do: Integer.floor_div(n, 100) + pass(current, current - Integer.mod(n, 100))
  def pass({"R", n}, current), do: Integer.floor_div(n, 100) + pass(current, current + Integer.mod(n, 100))
  def pass(current, n) when current > 0 and n < 0, do: 1
  def pass(_, n) when n > 100, do: 1
  def pass(_, _), do: 0
...
# Rotations.zero_dials_count("inputs/input01.txt") |> elem(0) |> IO.puts

# --- Part Two ---
Rotations.zero_dials_count("inputs/input01.txt") |> elem(1) |> IO.puts
